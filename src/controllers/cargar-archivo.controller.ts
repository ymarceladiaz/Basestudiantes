...
export class CargaArchivoController {
  ...
  async DocumentosPersona(
    ...
    const rutaDocumentoPersona = path.join(__dirname, llaves.carpetaDocumentosEstudiante);
    let res = await this.StoreFileToPath(rutaDocumentoPersona, llaves.nombreCampDocumentoEstudiante, request, response, llaves.extensionesPermitidasDOC);
    if (res) {
